...
def cost(target_pixel_data: tuple, source_pixel_data: tuple, vector: tuple) -> float:
    ...
    :param target_pixel_data: tuple
    :param source_pixel_data: tuple
    :param vector: tuple
    :return: float
    """
...
def best_cost_vector(target_im: Image, source_im: Image, loop_number: int) -> tuple:
    """
    ...
    best_vector: tuple[int, int, int] = random_vector(99)
    best_cost = cost(target_pixel_data, source_pixel_data, best_vector)
    costs = []
    for i in range(loop_number):
        current_vector = random_vector(99)
        current_cost = cost(target_pixel_data, source_pixel_data, current_vector)
...
def color_transfer(target_im: Image, source_im: Image, loop_number: int) -> Image:
    ...
    output_im = Image.new(target_im.mode, target_im.size)
...
def main(target_file_name: str, source_file_name: str, loop_number: int = 10) -> None:
    ...
    output_im = color_transfer(target_im, source_im, loop_number)
